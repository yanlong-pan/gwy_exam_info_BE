...
from utilities.encrypto import verify_hash
...
async def token(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if not verify_hash(constant.CLIENT_ID, client_id) or not verify_hash(constant.CLIENT_SECRET, client_secret):
...
async def get_current_client(token: str = Depends(oauth2_scheme)):
    try:
        ...
        if client_id is None or not verify_hash(constant.CLIENT_ID, client_id):
            raise credentials_exception
